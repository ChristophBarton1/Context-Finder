...
All four needed logos turned out to be single-path marks (sourced from
svgl.app / the brands' own favicons / CC0 Wikimedia — verbatim, not invented),
so `BrandIcon` only needs a `viewBox` override and a `fillRule` (Bolt's "b"
glyph has a counter that needs `evenodd` or it fills solid). No raw-SVG field
is required.

**Files:**
- Modify: `utils/icons.ts` (`BrandIcon` interface + `TARGET_ICONS`)
- Modify: `entrypoints/popup/App.svelte` (the `brandIcon` snippet)

- [ ] **Step 1: Extend `BrandIcon` in `utils/icons.ts`**
...
- [ ] **Step 2: Replace the chatgpt/lovable/bolt/base44 entries in `TARGET_ICONS`**
...
Replace with (real OpenAI blossom mark, single path, viewBox 256×260; no
color so it adapts to theme like other dark marks):
...
Replace with (real marks — Lovable's rounded-bubble silhouette in brand blue,
Bolt's "b" glyph with `evenodd` for its counter, Base44's banded-circle mark
in brand orange; Replit/v0 keep their simple-icons marks):
...
- [ ] **Step 3: Render `viewBox` + `fillRule` in the `brandIcon` snippet (`entrypoints/popup/App.svelte`)**
...
(The `monogram` and empty-slot branches are unchanged.)
...
```bash
git add utils/icons.ts entrypoints/popup/App.svelte
git commit -m "feat: real brand logos for ChatGPT, Lovable, Bolt, Base44"
```
